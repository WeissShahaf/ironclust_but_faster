...
- `matlab/irc.m`
  ...
  - Lines 5807-5839: `S_fig.csHelp` - added [0] key help text
  - Lines 6393-6399: `keyPressFcn_FigWav_()` - added [0] key to clear second cluster selection
  - Lines 7519-7522: `plot_FigMap_()` - use viSite2Chan for channel labels
  ...
  - Lines 23938-23949: `export_quality_zach_()` - defensive field sizing
  - Lines 23980-24007: `export_quality__()` - defensive field sizing, fix ypos bug
...
8. `struct_select_()` attempts to index `vrSnr_clu[105]` but the array only has 104 elements → **Error!**
...
## Issue 5: Table Size Mismatch in Quality Export

### Problem
When exporting cluster quality metrics to CSV using `irc export-quality`, users encountered an error:
```
Error using table
All table variables must have the same number of rows.
Error in irc>export_quality__ (line 23962)
```

**Root Cause:**
After merge/split/delete operations, some quality metric fields in `S_clu` had inconsistent sizes:
- `S.nClu` = 104 (number of clusters)
- `S.vrSnr_clu` = 103 elements (missing data for one cluster)
- `S.vrVpp_uv_clu` = 105 elements (extra data from previous state)
- etc.

Additionally, there were two bugs in the original code:
1. Both `xpos` and `ypos` were set to `S.vrPosX_clu` instead of `vrPosX_clu` and `vrPosY_clu`
2. Fields were not guaranteed to be column vectors with `(:)`

### Fix Location
**File:** `matlab/irc.m`
**Lines:** 23976-24007 (export_quality__), 23938-23949 (export_quality_zach_)
**Functions:** `export_quality__()`, `export_quality_zach_()`

### Changes Made

#### In `export_quality__()` (lines 23980-24005):
Added defensive sizing for all quality fields:
...
#### In `export_quality_zach_()` (lines 23942-23949):
Applied similar defensive sizing for the simplified export:
...
**Impact:**
- CSV export now works even after merge/split/delete operations with inconsistent field sizes
- All fields are guaranteed to be `[nClu x 1]` column vectors
- Missing values are zero-filled (or NaN for quality metrics)
- Extra values are truncated
- Fixed the `ypos` bug (was using `vrPosX_clu` instead of `vrPosY_clu`)

**Key Principle:**
Export functions should be defensive and handle inconsistent data sizes gracefully, as merge/split operations can temporarily create size mismatches.
